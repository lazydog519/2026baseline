...
\subsubsection{独立求解与验收协议}
两问的正式推理程序均以当前图、固定配置和冻结的全局规则为输入，
对每个图/核数从头生成方案；不读取其他图的方案、逐图成绩或评估器输出。
开发阶段可用官方程序检查模型误差，正式方案全部生成并记录
输入、代码和结果哈希后，再由分离脚本作 100 图全量验收。
因此本章报告的是可重复运行算法的结果，而非保存的逐例答案。
结论只对题目给定数据和硬件配置成立；更换带宽或缓存时，应重新
检查式~\eqref{eq:chapter6-a-proxy} 与式~\eqref{eq:chapter6-b-rank} 的排序误差。

图~\ref{fig:chapter6-mechanism} 先固定两问共有的硬件边界：
核数变化只增加并行计算资源，不增加 DDR 总带宽；L1/UB 则由各核私有。
图中 Task 和 COPY 的箭头表示题目规定的数据通路，
不是对某一用例实测时间线的描摹。
\begin{figure}[htbp]
\centering
\includegraphics[width=0.94\linewidth]{论文第六章/figures/图6-1_场景与资源机制.pdf}
\caption{场景 A、B 的 Task 边界与数据通路，以及多核就绪 COPY 请求对共享 DDR 带宽的争用。等待周期、容量与带宽均取题目固定配置；底部是规则示意，不预设 FIFO 顺序，也不表示实测排队时长。}
\label{fig:chapter6-mechanism}
\end{figure}
...
$E_{OO}$ 是原图中直接连接两计算操作的依赖集合，$d_{uv}$
...
$b_{uv}$ 为该依赖关联的张量字节数与直接边 $d_{uv}$ 之和
（若无直接边则 $d_{uv}=0$）。
沿原操作 DAG 递推
\begin{align}
\widehat F_v^B&=c_v+
...
\end{tabular}
\end{table}
五核时场景 B 的平均加速比为 3.9522，高于场景 A 的 3.8311。逐图配对比较中，B 在 31 图耗时缩短、19 图增加、50 图相同；以 A 的逐图耗时为分母，B 的平均耗时缩减率为 +1.67\%，平均额外搬运差为 -5.529 MiB。这些是同一批图的描述统计，不能据此断言每个图上 B 都更快。
\begin{figure}[htbp]
\centering
\includegraphics[width=0.86\linewidth]{论文第六章/figures/图6-2_两场景平均加速比.pdf}
\caption{场景 A 与 B 在 100 图上的 1--5 核平均加速比。单核值按题意定义为 1；其余点各为 100 图算术平均。}
\label{fig:chapter6-speedup}
\end{figure}
图~\ref{fig:chapter6-speedup} 显示两场景的平均加速比均随核数增加，场景 B 在 2--5 核的每个核数上均较高。该曲线报告总体均值，不能代替逐图分布；单核的 1 是定义值而非另一次多核实验。
\begin{figure}[htbp]
\centering
\includegraphics[width=0.86\linewidth]{论文第六章/figures/图6-3_额外DDR搬运分解.pdf}
\caption{2--5 核的平均额外 DDR 搬运量。斜线段是原版评估程序给出的缓存换入/换出新增字节，其余部分是总额外搬运扣除该字段；后者不全部等同于跨核搬运。每根柱对应 100 图的算术平均。}
\label{fig:chapter6-copy}
\end{figure}
五核时缓存换入/换出所致新增搬运在 A、B 下的平均值分别为 7.423 和 5.315 MiB。图~\ref{fig:chapter6-copy} 说明场景 B 的总额外搬运较低，但两场景的溢出成本都不可忽略；因此不能把同核直通写成“缓存必定无溢出”。
\begin{figure}[htbp]
\centering
\includegraphics[width=0.86\linewidth]{论文第六章/figures/图6-4_逐图加速比分布.pdf}
\caption{两场景在 2--5 核下的逐图加速比分布。每箱含 100 图；箱体为第 25--75 百分位，中线为中位数，须延至距四分位数1.5 倍四分位距内的最远观测，圆点为其外观测；箱体不是均值置信区间。}
\label{fig:chapter6-box}
\end{figure}
图~\ref{fig:chapter6-box} 给出均值曲线之外的离散程度。五核时 A、B 的逐图中位加速比分别为 4.043、4.148；结合 31 图改善、19 图退步、50 图持平的配对结果，场景 B 的收益存在结构依赖，不能从场景规则推出逐图单调改进。模型中的驻留风险只用于求解阶段的候选筛选，本节所有性能数值均来自方案冻结后的官方验收。
% Q2_RESULTS_END
