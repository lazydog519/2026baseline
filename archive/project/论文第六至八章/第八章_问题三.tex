...
\algline{置时刻 $t\leftarrow0$；初始化剩余前驱数、就绪队列和完成事件队列。}
...
\algline[1]{取出释放时刻不晚于 $t$ 的就绪事件；读写按式~\eqref{eq:q3-route} 入池，计算按 $c_u$ 登记完成时刻。}
\algline[1]{由式~\eqref{eq:q3-next} 求两池最近完成间隔，空池取 $+\infty$。}
\algline[1]{取就绪释放、计算完成和传输完成中的最早时刻 $t'$；若均不存在，报告代理依赖环。}
\algline[1]{按 $\Delta t=t'-t$ 扣减两池剩余字节，置 $t\leftarrow t'$，收集本时刻已完成事件。}
...
\algline{\textbf{return} $t$ 与累计服务字节数。}
\end{algorithmblock}
