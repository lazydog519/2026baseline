...
FILES=[(6, ROOT/'第六章_问题一.tex'),(7, ROOT/'第七章_问题二.tex'),(8, ROOT/'第八章_问题三.tex')]
...
def main():
    ...
    for n,s in sources.items():
        ...
        s=re.sub(r'\\section\{([^}]+)\}',lambda m:'# 第'+{6:'六',7:'七',8:'八'}[n]+'章　'+m[1],s)
        count=[0]
        def sub(m):
...
'''
    (ROOT/'第六至八章_修订稿.md').write_text('\n\n'.join(outputs)+refs,encoding='utf-8')
    (ROOT/'第八章_问题三_阅读版.md').write_text(outputs[-1]+'\n',encoding='utf-8')
    print(f'equations={len(eqs)} figures={len(figs)} tables={len(tabs)} algorithms=4')
