...
活跃区间 $I_{t,k}$。分别估计 L1 与 UB 的逻辑峰值和超容量量
\begin{equation}
...
问题二、三加入独立分量的输入复用排序候选。
合并后重新检查收缩图无环。为获得简单可靠的构造，本实现额外限制
核心依赖图无环；这是一项保守的搜索空间限制，不是题目要求。
每次输入重新构造最多十个结构候选，去重后计算特征，避免无界枚举。
...
\subsection{实验隔离与指标}
开发阶段允许官方评估提供标签；冻结阶段仅携带两个推理源码和本轮参数，
