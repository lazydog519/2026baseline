...
def describe(g, plan, cfg, question):
    ...
    for v in topo(pred, succ):
        start[v] = max((finish[p] + ((cross if question == 1 else sync) + 2 * g.edge_bytes[p, v] / bw
    ...
    for t, tensor in g.tensors.items():
        ...
        writes = (int(bool(remote) or t in g.outputs) if question == 1 else
                  len(remote) + int(t in g.outputs)) if sources else 0
        reads = len(remote)
