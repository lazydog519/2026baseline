...
PROJECT = ROOT.parent
RUN = PROJECT / 'q3_adaptive_20260926'
...
def performance(rows, summary):
    ...
    with (PROJECT/'q2_priority_20260926/final_v2/full_metrics.csv').open(encoding='utf-8-sig') as f:
...
def windows(rows):
    ...
    case=best['case']
    path=RUN/'full5_v2/results'/f'{case}_n5'/'l2.json.gz'
...
def main():
    ...
    summary=json.loads((RUN/'final/aggregate.json').read_text('utf-8'))
    ...
    audit=dict(paired_jobs=500,official_evaluations=1000,all_added_copy_bytes_equal=True,
        ...
        trace=trace,display_table=table,figures=3,formats=['png','pdf','svg'],
