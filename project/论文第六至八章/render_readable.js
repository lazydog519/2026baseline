...
const root=__dirname, name=process.argv[2]||'第六至八章_修订稿', base=path.join(root,name);
...
(async()=>{
 ...
 try{
  ...
  if(check.errors||check.images!==(single?3:6)||check.algorithms!==(single?2:4)||check.headings.length!==(single?1:3)||check.equationOverflow)throw Error(JSON.stringify(check));
