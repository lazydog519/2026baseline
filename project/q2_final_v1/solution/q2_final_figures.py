...
def main():
  ...
  # 5-7: balanced five-core diagnostic case_075; exact official evaluator traces.
  ex=pkg/'results/examples';initial=loadgz(ex/'case_075_n5_initial.json.gz');final=loadgz(ex/'case_075_n5_selected.json.gz')
  ...
  ax.set(xticks=[0,1],xticklabels=['Initial Q2 plan','Selected CILC plan'],ylabel='Moved data (MiB)',title='Case 075: official traffic decomposition');ax.legend(frameon=False,fontsize=8);ax.grid(axis='y',alpha=.16);save(fig,out,'q2_fig5_case075_traffic')
  pipes=['PIPE_M','PIPE_V','PIPE_MTE2','PIPE_MTE3'];pc=[BLUE,GREEN,ORANGE,'#E7A269']
  for idx,(result,label,name) in enumerate([(initial,'Initial Q2 plan','q2_fig6_case075_initial_timeline'),(final,'Selected CILC plan','q2_fig7_case075_selected_timeline')]):
    ...
    ax.set(yticks=[c*4+j for c in range(result['num_cores']) for j in range(4)],yticklabels=[f'Core {c} · {p.replace("PIPE_","")}' for c in range(result['num_cores']) for p in pipes],xlabel='Simulator time (kcycles)',title=f'Case 075: {label} · makespan {result["makespan"]:,} cycles',xlim=(0,result['makespan']/1000*1.025));ax.invert_yaxis();ax.grid(axis='x',alpha=.16);ax.tick_params(axis='y',labelsize=8,length=0);ax.axvline(result['makespan']/1000,color=GRAY,ls='--',lw=.8);ax.legend(handles=[Patch(facecolor=pc[j],label=pipes[j].replace('PIPE_','')) for j in range(4)],loc='upper right',ncol=4,frameon=True,fontsize=8);save(fig,out,name)
