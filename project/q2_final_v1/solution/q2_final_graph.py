"""Q2-local DAG constructors. Adapted generic graph utilities; no Q1 outputs are read."""
...
from q2_final_baseline import make_plan as baseline_plan
...
def terminal_branch_plan(graph, num_cores):
    ...
    shared = {oid for oid in ops if bin(sink_bit[oid]).count("1") > 1}
    ...
    for band in sorted(bands):
        ...
        for oid in reversed(order):
            if oid in ids:
                for nxt in succ[oid] & ids:
                    mask[oid] |= mask[nxt]
        shared = {oid for oid in ids if bin(mask[oid]).count("1") > 1}
        load = [[0, 0] for _ in range(num_cores)]
