...
VERSION = 'fresh-q3-20260926-v1'
FEATURES = ['compute_bottleneck', 'cross_path_exposure', 'compulsory_io',
...
class Graph:
    ...
    def features(self, plan, cfg):
        ...
        values = [bottleneck, max(0, max(exposed.values(), default=0) - bottleneck),
...
def solve(raw, n, cfg, weights, method='trained'):
    ...
    if method == 'component_scalar':
        ...
        selected = min(candidates, key=lambda c: (sum(a * b for a, b in zip(weights, c['features'])),
