...
def solve(graph, cores, official_code, fixed_config, search, target, trace):
    ...
    for _,name,w,plan in sorted(candidates,key=lambda x:(x[0],x[1])):evaluate(name if w==0 else f'{name}_w{w}',plan)
    if best is None:raise RuntimeError('No legal initial candidate')
    if search['greedy_reschedule']:evaluate('greedy_schedule',model.greedy(best))
    if search['greedy_merge']:evaluate('greedy_coarsen',model.coarsen(best,search['merge_limit'],search['merge_pair_trials']))
    ...
    report=dict(schema=search['version'],cold_start=True,cores=cores,search_config=search,initial_width=width,
                ...
                stop_reason='fixed_candidate_budget',history_read=False)
...
def main():
    ...
    r=solve(json.loads(a.graph.read_text(encoding='utf-8')),a.num_cores,a.official_code,a.config or a.graph.with_name('config.txt'),
