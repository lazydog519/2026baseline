...
class Model:
    ...
    def cache_pack(self, cores, capacity):
        """Greedily group independent components under a conservative footprint.

        A single large component is allowed through for the official spill rules;
        this footprint is a packing rule, never a feasibility test for the task.
        """
        parent={o:o for o in self.ops}
        def find(o):
            while parent[o]!=o:parent[o]=parent[parent[o]];o=parent[o]
            return o
        for o in self.order:
            for x in self.succ[o]:
                a,b=find(o),find(x)
                if a!=b:parent[max(a,b)]=min(a,b)
        members=defaultdict(list);touched=defaultdict(set)
        for o in self.ops:members[find(o)].append(o)
        tensors={t['id']:t for t in self.g['tensors']}
        inputs=set();outputs=set()
        for tid in tensors:
            ps=self.prod[tid] & self.ops.keys();cs=self.cons[tid] & self.ops.keys()
            for o in ps|cs:touched[find(o)].add(tid)
            if cs and not ps:inputs.add(tid)
            if ps and (not cs or any(self.allops[o]['op']=='COPY_OUT' for o in self.cons[tid])):outputs.add(tid)
        pieces=[]
        for root,ids in members.items():
            work=defaultdict(int)
            for o in ids:work[self.ops[o]['pipe']]+=self.ops[o]['cycles']
            pieces.append((max(work.values()),root,ids,dict(work),touched[root]))
        opened=[None]*cores;free=[0.]*cores;schedules=[[] for _ in range(cores)];mapping={};next_id=0
        size=lambda ids:sum(tensors[x]['size'] for x in ids)
        def state(ids,work,ts):
            mem={r:sum(tensors[x]['size'] for x in ts if ('UB' if tensors[x]['pos']=='DDR' else tensors[x]['pos'])==r) for r in capacity}
            io=size(ts & inputs)+size(ts & outputs)
            return dict(ids=ids,work=work,tensors=ts,memory=mem,io=io,duration=max(max(work.values(),default=0),io/self.bw))
        def seal(k):
            nonlocal next_id
            s=opened[k]
            if s is None:return
            mapping.update({str(o):next_id for o in s['ids']});schedules[k].append(next_id);next_id+=1
            free[k]+=s['duration']+(self.same if len(schedules[k])>1 else 0)
            opened[k]=None
        for _,root,ids,work,ts in sorted(pieces,key=lambda x:(-x[0],x[1])):
            alone=state(ids,work,ts);choices=[]
            for k,current in enumerate(opened):
                merged=alone;start=free[k]+(self.same if schedules[k] else 0);fresh=current is not None
                if current is not None:
                    combined=dict(current['work'])
                    for q,w in work.items():combined[q]=combined.get(q,0)+w
                    merged=state(current['ids']+ids,combined,current['tensors']|ts)
                    if any(merged['memory'][r]>capacity[r] for r in capacity):
                        merged=alone;start+=current['duration']+self.same
                    else:fresh=False
                choices.append((start+merged['duration'],merged['io']-(current['io'] if current and not fresh else 0),k,fresh,merged))
            _,_,k,fresh,merged=min(choices,key=lambda x:x[:3])
            if fresh:seal(k)
            opened[k]=merged
        for k in range(cores):seal(k)
...
def solve(graph, cores, official_code, fixed_config, search, target, trace):
    ...
    if search['cache_packing']:
        plan=model.cache_pack(cores,cfg['capacity'])
        if len(set(plan['node_to_subgraph'].values()))<=search['max_generated_tasks']:
            candidates.append((model.estimate(plan),'cache_pack',0,plan))
        else:emit(dict(name='cache_pack',status='construction_budget_skipped'))
    candidates=sorted(candidates,key=lambda x:(x[0],x[1]))[:search['structure_evaluations']]
