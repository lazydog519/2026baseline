...
const source = path.join(root, "问题三_直观阅读版.md");
const htmlFile = path.join(root, "问题三_直观阅读版.html");
const pdfFile = path.join(root, "问题三_直观阅读版.pdf");
const formulas = [];
...
(async function () {
  ...
  try {
    ...
    if (check.errors || check.images !== 2 || check.totalImages !== 2) {
