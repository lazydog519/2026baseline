...
def main():
    ...
    for q, ax in zip((1, 2, 3), axes):
        ...
        ax.set(xticks=range(1, 6), xlabel='核心数', ylabel='同方案无 L2 / 有 L2 完成时间' if q == 3 else '平均加速比（原图单核参考）')
    ...
    left.legend(frameon=False, fontsize=8, ncol=3, loc='lower right')
    path = HERE / 'validation/case_026/q3_n5/trained/official.json.gz'
    ...
    right.set_title('(b) case 026：实际插入与命中事件', loc='left', pad=10)
    ...
    paired = selected[3, 5, 'case_026', 'trained']['same_plan_cache_gain']
    fig.supxlabel(f'例图由真实验收轨迹绘制：字节命中率 {stat["hit_rate"]:.2%}，同方案 Cache 加速比 {paired:.4f}；命中量不等于时间收益。', fontsize=9)
    plotdata['cache_trace'] = dict(case='case_026', time_kcycles=t, used_kib=v, hit_times_kcycles=hits,
                                   source='validation/case_026/q3_n5/trained/official.json.gz')
    ...
    text = ['全部 **180/180** 份方案通过官方验收。140 次开发标注与 162 次验收调用分属两个阶段；验收阶段对完全相同的方案复用评分，仅避免重复计算。官方文件 114 份保持原始哈希，模型与推理代码均保持冻结。',
