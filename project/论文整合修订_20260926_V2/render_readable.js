...
const root=__dirname, name=process.argv[2]||'整合修订稿', base=path.join(root,name);
const single=name==='第八章_问题三_阅读版';
...
.references{break-inside:avoid;font-size:11px;line-height:1.25}.references h2{font-size:12px;margin:6px 0 3px}.references p{font-size:8pt;line-height:1.25;margin:2px 0;text-indent:0;text-align:left}
@media print{body{max-width:none;margin:0;padding:0;font-size:12pt;line-height:1.35}h1{font-size:14pt;text-align:center;break-before:auto;margin-top:22px}h2{font-size:12pt}table{font-size:9.7pt}figcaption{font-size:9pt}.algorithm{font-size:9.7pt;line-height:1.55}.algtitle{font-size:10.5pt}p{margin-bottom:8px}}
.paper-title{font:700 24px/1.5 SimHei,sans-serif;text-align:center;margin:10px 0 24px}.abstract-title{text-align:center}.references{break-inside:auto}.references p{font-size:10.5pt;line-height:1.35;overflow-wrap:anywhere}.references h2{font-size:12pt;margin:20px 0 10px}
@media print{.paper-title{font-size:16pt}h1:first-of-type{break-before:page}figure{margin:12px 0}figcaption{line-height:1.35}}
...
(async()=>{
 ...
 try{
  ...
  await page.evaluate(()=>document.querySelectorAll('p').forEach(p=>{if(p.nextElementSibling && p.nextElementSibling.classList.contains('equation'))p.style.breakAfter='avoid';}));
  ...
  check.tableOverflow=[...await page.locator('table').evaluateAll(els=>els.filter(x=>x.scrollWidth>x.clientWidth+3).map(x=>(x.querySelector('caption') ? x.querySelector('caption').textContent : null)))];
  if(check.tableOverflow.length||check.errors||check.images!==10||check.algorithms!==4||check.headings.length!==9||check.equationOverflow)throw Error(JSON.stringify(check));
  ...
  fs.writeFileSync(base+'_排版检查.json',JSON.stringify(check,null,2)+'\n');
