...
def source_hashes():
    return {f: digest(HERE / f) for f in ('solver.py', 'mechanism.py', 'stage_candidates.py')}
...
def generate():
    ...
    with tempfile.TemporaryDirectory(prefix='npu_fresh_') as temp:
        root = Path(temp)
        for file in ('solver.py', 'mechanism.py', 'stage_candidates.py', 'model.json'): shutil.copy2(HERE / file, root / file)
    ...
    with zipfile.ZipFile(HERE / 'submission_solver.zip', 'w', zipfile.ZIP_DEFLATED) as z:
        for file in ('solver.py', 'mechanism.py', 'stage_candidates.py', 'model.json'): z.write(HERE / file, file)
