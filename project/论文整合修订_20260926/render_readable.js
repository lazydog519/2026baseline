...
(async()=>{
 ...
 try{
  ...
  await page.evaluate(()=>document.querySelectorAll('p').forEach(p=>{if(p.nextElementSibling?.classList.contains('equation'))p.style.breakAfter='avoid';}));
  ...
  check.tableOverflow=[...await page.locator('table').evaluateAll(els=>els.filter(x=>x.scrollWidth>x.clientWidth+3).map(x=>x.querySelector('caption')?.textContent))];
