"""Independent scene-B plus FIFO L2 solver. No saved plan or cross-case state.
...
from q1_optimized import compute_dag, topological_depth, depth_band_plan, valley_stage_plan

VERSION = "q3-cold-fusion-v1"
DEFAULT_CONFIG = Path(__file__).with_name("q3_cold_config.json")
...
class Problem:
    ...
    def branch_candidates(self, plan, limit):
        """Split a shared-entry subgraph into independent weak branches.

        Root operations stay together. Each remaining weak component is an
        atomic branch, so the structural proposal preserves every operation
        and cannot create inter-branch dependency edges.
        """
        mapping, assignment, groups = self.view(plan)
        _, _, load = self.estimate(plan)
        ranked = sorted(groups, key=lambda s: (-sum(self.ops[o]["cycles"] for o in groups[s]), s))
        emitted = 0
        for s in ranked:
            within = set(groups[s])
            roots = {o for o in within if not self.pred[o] & within}
            if not 2 <= len(roots) <= 64:
                continue
            ...
            while remaining:
                ...
                while stack:
                    o = stack.pop()
                    members.add(o)
                    for v in sorted((self.pred[o] | self.succ[o]) & remaining):
                        remaining.remove(v)
                        stack.append(v)
                components.append(members)
            if not 2 <= len(components) <= 32:
                continue
            if len(groups) + len(components) > 256:
                continue
            p = {"node_to_subgraph": dict(plan["node_to_subgraph"]),
                 "core_schedules": [list(seq) for seq in plan["core_schedules"]]}
            for o in roots:
                p["node_to_subgraph"][str(o)] = s
            current_load = [list(x) for x in load]
            source = assignment[s]
            for part in components:
                for o in part:
                    j = self.ops[o]["pipe"] == "PIPE_V"
                    current_load[source][j] -= self.ops[o]["cycles"]
            for idx, part in enumerate(sorted(components, key=lambda x: -sum(
                    self.ops[o]["cycles"] for o in x)), start=1):
                new_s = max(groups) + idx
                weight = [sum(self.ops[o]["cycles"] for o in part if self.ops[o]["pipe"] == pipe)
                          for pipe in ("PIPE_M", "PIPE_V")]
                target = min(range(self.n), key=lambda c: (max(current_load[c][i] + weight[i]
                                                               for i in range(2)), c))
                current_load[target][0] += weight[0]
                current_load[target][1] += weight[1]
                for o in part:
                    p["node_to_subgraph"][str(o)] = new_s
                p["core_schedules"][target].append(new_s)
            try:
                yield f"root_branch_s{s}", self.reorder(p, "stable")
                emitted += 1
            except ValueError:
                continue
            if emitted >= limit:
...
def solve(graph, cores, official, fixed_config, search, output, trace_dir):
    ...
    cache = read_cache_config(str(fixed_config))
    ...
    def offer(name, plan, family, descriptor=None):
        ...
        pending[fp] = dict(name=name, plan=plan, family=family, descriptor=descriptor,
                           ...
                           lower_bound_cycles=max(problem.compute_path_bound, max(map(max, load))))
    ...
    def evaluate(candidate):
        ...
        try:
            result = evaluate_problem_3(graph, candidate["plan"], bandwidth=settings["bandwidth"],
    ...
    while calls < search["max_evaluations"] and time.perf_counter()-start < search["max_seconds"]:
        ...
        if best is not None and fingerprint(best) != local_parent:
            ...
            for name, plan in problem.branch_candidates(best, search["branch_groups"]):
                offer(name, plan, "local")
        ...
        selected = min(choices, key=lambda c: (c["proxy_cycles"], c["pre_spill_bytes"], c["name"], c["fingerprint"]))
        evaluate(selected)
    if best is None:
...
def main():
    ...
    parser.add_argument("-n", "--num-cores", type=int, required=True, choices=range(1, 6))
    parser.add_argument("--official-code", type=Path, default=Path(__file__).resolve().parents[1]/"official"/"code")
