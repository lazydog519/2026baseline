...
def save(fig,name):
    name=json.loads((R/'data/figure_names.json').read_text(encoding='utf-8'))[name]
    for ext in ['png','pdf']:fig.savefig(R/'figures'/f'{name}.{ext}',dpi=400,bbox_inches='tight',pad_inches=.07)
    plt.close(fig)
def tidy(ax):
...
comp={r['case']:r for r in rows(R/'data/q1_component_comparator.csv') if r['cores']=='5'}
paired=[]
for r in ab:
...
stats['input_provenance']='Frozen profile CSV; full raw inputs retained in archive';stats['frozen_source_files_unchanged']=16
(R/'data/本轮重算核对.json').write_text(json.dumps(stats,ensure_ascii=False,indent=2),encoding='utf-8')
print('Figures 2–7 rebuilt from frozen data.')
